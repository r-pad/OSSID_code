'''
For computing the detection metrics
'''
...
class DetectionMetric():
    def __init__(self, classes, device=None):
        ...
        '''
        self.label_map = {k: v + 1 for v, k in enumerate(classes)}
        self.label_map['bg'] = 0
    ...
    def calculate_mAP(self, det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties=None):
        """
        ...
        """
        # If true_difficulties is None, by default set all detection to be not difficult. 
        if true_difficulties is None:
            true_difficulties = []
            for i in range(len(true_labels)):
                true_difficulties.append(torch.zeros_like(true_labels[i]))
        ...
        average_precisions = {self.rev_label_map[c + 1]: v for c, v in enumerate(average_precisions.tolist())}
